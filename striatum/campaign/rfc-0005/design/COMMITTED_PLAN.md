...
# COMMITTED_PLAN — RFC 0005: Exporter-fed capacity signal (probe-anchored)

author: committer-claude-opus-4.8-001

This is the **gated, committed build plan** for RFC 0005. It is the holder's cycle-3
build plan (`dialogue/holder/BUILD_PLAN.md`, `holder-claude-opus-4.8-003`, artifact
`art_f80ac4a35f9a425e1df55b242225e8c9`) **amended with every binding constraint** the
adjudicator recorded — the exact contract the build run will execute. It is
self-contained: the ordered, independently-committable slices and their blast radius, the
Migration `010` schema + apply order, the falsifiable-gate → test map with each binding
constraint folded in, the live-infra and `di --json` boundaries, and the operator
deployment steps. (The RFC body's *"Migration 010 (or 011 …)"* and the workflow
objective's generic *"migration 006"* phrasing both resolve to **`010`**, the lowest
unused number in the live `migrations/` tree — see §2 of the embedded plan.)

## Gate clearance (the clearing instrument this plan commits to)

The RFC 0005 design gate cleared by an **operator decision that supersedes the
adjudicator's `needs_revision`**, not by a native `accept`/`accept_with_findings`
adjudication. Both records are preserved here faithfully; neither is hidden.

- **Adjudicator ledger (cycle 3):** `dialogue/adjudicator/COLLABORATION_LEDGER_cycle_3.md`
  (`adjudicator-claude-opus-4.8-004`, artifact `art_9bac54b89e44c431ad9fc7a062336a9b`),
  run `run_0e0a6f7601cc744dec24f48b43bea9e1`, workflow `rfc-0005-design`, cycle 3.
  **Verdict: `needs_revision`** — one **`landed_unrebutted`** blocker (**BC1**: the
  reader-side headroom invariant has no production path, `kv_bytes` is an undefined symbol,
  and the `di --json` boundary is unreconciled) plus four accompanying repairs
  **BC2–BC5**. The ledger's own classification is explicit and load-bearing for this
  commit: the defect is **dischargeable in one build cycle, NOT undischargeable** (hence
  `needs_revision`, never `reject`); the **design spine survived falsification intact**
  (its "What survived" block); and the gate was **stuck only because the workflow revision
  loop (`cycles[0].max_iterations = 2`) was spent** with both routed cycles producing no
  holder revision on the gated trajectory — a template routing limitation recorded as the
  process finding `f_revision_loop_exhausted`, **not** a design rejection.
- **Operator decision (the clearing instrument):** `OPERATOR_DECISION_BC1_override.md`
  (`decision_id: rfc-0005-design-override-bc1`, artifact
  `art_195a2db1e37a5750c9ad55af663be8c5`, `owner: human`, `created_at 2026-06-26T23:46:34Z`),
  **outcome `accepted_with_follow_up`**. It **supersedes** the cycle-3 `needs_revision` on
  exactly the grounds the ledger recorded — *cycle budget exhausted, design spine sound,
  BC1 dischargeable in one cycle without re-opening the settled RFC* — and **directs the
  committer to fold BC1–BC5 into this committed plan as binding constraints**, the build to
  implement BC1 and its e2e 32k-vs-4k falsifying test, and the independent build verifier
  to confirm BC1 and BC2 gate tests before `accept`. This operator decision is the
  **clearing instrument** that authorizes the commit phase.
- **Why this is honest, not a forced clear:** the operator decision does **not** relabel
  BC1 as rebutted (which would be a fabrication and is mechanically refused at publish under
  RFC 0094 §5 Check-B). It instead **accepts BC1 as a binding BUILD constraint** and moves
  its *discharge* into the build stage under the independent falsifiable-gate verifier —
  precisely the resolution path the adjudicator named (re-task BC1 as a build constraint,
  preserve the spine, do not re-open the RFC). Every "What survived" invariant is preserved
  intact.

**Note on the embedded plan's discharge state.** The holder revised three times; the
**on-branch / latest** holder plan committed here (`holder-claude-opus-4.8-003`,
65,402 bytes) **already folds BC1–BC5 into its slices and its §3 gate→test map** and
additionally hardens against three code-grounded defects the attempt-2 falsifiers raised
(**F-CARD** policy-join cardinality, **F-KEYS** shared-UPSERT `KeyError`, **F-LOCK**
`FOR UPDATE` through a view) plus a baseline-robustness reinforcement (**F-BASE**). The
cycle-3 adjudicator scored an earlier byte-identical trajectory and so did not see this
revision (the source of `f_revision_loop_exhausted`); the operator decision resolves that
routing gap. **BC1–BC5 are the adjudicator's binding constraints and gate this build;
F-CARD/F-KEYS/F-LOCK/F-BASE are the holder's additional self-hardening, carried in the
body and not weakened.**

**Settled, do-not-reopen design** (RFC `docs/rfc/0005-exporter-capacity-signal.md` + the
ledger spine — preserve intact through the build): **C-EPOCH** (fast capacity bands live
only in the companion table and **never bump `gpu_slots.epoch`**, gating NEW picks/claims
only; only the slow capability bands `mig_mode`/`ecc_mode` bump `epoch` and fence held
leases — dissolving the self-abort loop); **C3** companion-table fault isolation (LEFT JOIN
+ a separate savepoint-guarded write mirroring `NODE_LEASE_CAS`); **C4**
`effective_free = LEAST(probe_floor, exporter_free)` probe-anchoring; **OQ-P** phantom
shrinks `effective_free` rather than minting a self-lease; **C5** fleet-floor dead-man
guard (`pick` never returns empty); **C1/C2** additive/reversible Migration `010` at the
...
Every constraint below is preserved **exactly as the adjudicator recorded it** in the
cycle-3 ledger `constraints[]`; **none is weakened or dropped**. The operator decision
accepted all five as binding. **BC1 and BC2** are gate constraints the **independent build
verifier MUST confirm before `accept`** (`binding: true`, `final_review_required: true`);
**BC3, BC4, BC5** are policy constraints carried as must-pass build tests (`binding:
false`, `final_review_required: false`). The embedded holder plan **already discharges all
five in plan-text**; the **build implements them and the verifier gates on BC1 + BC2**.

| BC | Sev / kind | Binding | Discharge status | The contract the build MUST satisfy | Proving test(s) (embedded §3) | Verifier final review |
|----|-----------|---------|------------------|-------------------------------------|-------------------------------|-----------------------|
| **BC1** | high / gate | **yes** | plan-discharged → **build-implement + verify-gate** | The RFC headroom invariant is enforced **in production**, not just in SQL: `model_footprint`(`model_mib`) + request `max_context` sourced at the di-fleet layer (argv `--model`/`--max-context` via `_split_argv` + a registry `model_capacity` model→mib/KV policy row) **without** importing the DI/Node engine or probing hardware (escalate if neither suffices); `kv_bytes` is a **defined** inline SQL expression created+tested in **Slice 3**; the **same** inputs thread through `route_slots`/pick, the first-attempt `run_leased_shard`/`claim`, **and** `run_failover_shard`/`failover_transfer`/`claim` (a defaulted kwarg production never populates does NOT satisfy this); C7 restated for production threading; the `di --json` boundary preserved (argv + registry SQL only). | **§3 row 8** — N1 e2e `di_fleet` 32k-vs-4k (hermetic), N2 PG `LEASE_CLAIM_SQL`/`PICK`, N3 no-engine-import boundary | **required** |
| **BC2** | high / gate | **yes** | plan-discharged → **build-implement + verify-gate** | Freshness decay is **single-clock**: staleness from same-clock summands (a node-clock `fast_source_age_s` + a DB-clock `now() − updated_ts`), **never** node-clock `fast_source_ts` vs DB-clock `heartbeat_ts`; **do NOT** stamp `source_ts` with DB `now()` (that defeats frozen-exporter detection, RFC gate bullet 1); OQ-C restated naming each clock; **both** a skew-resistance test and a frozen-source-decay test. | **§3 row 1** — A1/A2 hermetic (frozen + skew), A3/A4 PG (frozen-decays-out-of-pick + DB-skew-stays-measured) | **required** |
| **BC3** | medium / policy | no | plan-discharged → verify-carry | `live_slowdown_factor` guarded against `probe_ms` None (failed probe; and the residency-only `ollama-ondemand` path, None every tick by design) and `cold_probe_ms` None/0 — write NULL/sentinel + `capacity_source` absence, **never raise**, inside the savepoint-guarded boundary; the `ollama-ondemand` no-baseline rule defined (a well-formed companion row, never a crash, never force-loaded); failed-probe + cold-`ollama-ondemand` tests. | **§3 row 6** — K/K2 hermetic, K3 PG (NULL + sticky-baseline) | not required |
| **BC4** | medium / policy | no | plan-discharged → verify-carry | The companion `CAPACITY_UPSERT` is wired into the **puller** write-path (`heartbeat_all.py` `pull_write`/`tick`) under the same separate-savepoint-guarded discipline so pull-mode slots — **including peecee**, Principle 3's motivating co-tenant host — get `gpu_slots_capacity` rows; "only the card-owning node writes its phantom" honored; a puller integration test proves the companion row is written. | **§3 row 4** — H/M2 hermetic, I/M PG (puller writes companion) | not required |
| **BC5** | low / policy | no | plan-discharged → verify-carry | Disambiguate that slices are independently **committable** (green under hermetic pytest, which applies all migrations) but **NOT** freely deploy-ordered: Slice 2 has a **HARD deploy-order precondition on Migration 010** (`mig_mode`/`ecc_mode` ride the **non**-savepoint-guarded liveness UPSERT); **DB→writer→reader** is the operative apply order; `mig`/`ecc` stay in the `gpu_slots` UPSERT epoch `CASE` (do NOT move them into the guarded companion write); no runtime column-existence probing. | **§1** (committable-vs-deployable) + **§2** (DB→writer→reader) | not required |

### Verbatim binding-constraint text (preserved exactly; do not weaken or drop)

> **BC1** *(severity high, kind gate, binding: true, source_finding `f_reader_request_capacity`)* —
> BLOCKING repair (the reason the gate cannot clear). Slice 3 MUST define the
> request-capacity contract explicitly, with a discharging test, such that the RFC's
> headroom invariant is enforced in PRODUCTION, not just in the SQL: (a) di_fleet MUST
> obtain model_footprint (model_mib) and request max_context at the di-fleet layer — from
> the already-handled --model / context argv (di-fleet already splits these in _split_argv)
> and/or a registry-side model→mib policy row — WITHOUT importing DI/Node engine internals
> or probing live hardware; if neither source suffices the build MUST escalate rather than
> cross the di --json boundary or measure real GPUs. (b) Define kv_bytes EXPLICITLY and
> name its owning slice: a SQL function / generated expression created in Migration 010, a
> capacity_policy lookup, or a Python helper — the Slice 3 SQL must reference only symbols
> that actually exist, and the chosen form must be created AND tested in a named slice.
> (c) Thread the SAME capacity inputs (model_mib + the max_context-derived KV budget)
> through route_slots()/pick, the first-attempt run_leased_shard()/claim, AND
> failover_transfer()/run_failover_shard()/claim — a defaulted optional kwarg that
> production never populates does NOT satisfy this. (d) Add an end-to-end hermetic test of
> di_fleet.main() (or the nearest production orchestration surface) proving a high-context
> (e.g. 32k) request and a low-context (4k) request route DIFFERENTLY against the SAME slot
> whose effective_free sits between the two headroom thresholds, asserting that pick and
> BOTH claim paths receive non-default capacity inputs and that kv_bytes resolves to a
> defined symbol. RESTATE C7 so it covers the production threading, not only the
> empty-companion COALESCE fall-through. The di --json boundary MUST be preserved (parse
> argv + registry SQL only; never import the engine).
> **Verification gate:** e2e di_fleet.main() test: a 32k request is refused and a 4k request
> accepted on the SAME slot whose effective_free is between the two headroom thresholds;
> pick + first-attempt claim + failover claim all receive non-default model_mib and
> max_context; kv_bytes resolves to a defined symbol; no engine import, no live-hardware
> read. **Expected stage:** design_revision_then_build_verify. **final_review_required: true.**

> **BC2** *(severity high, kind gate, binding: true, source_finding `f_freshness_clock_skew`)* —
> The freshness-decay comparison MUST be SINGLE-CLOCK. Staleness MUST be computed from two
> timestamps sampled on the SAME clock — e.g. node-local now − exporter/probe source_ts,
> both measured on the writing node (written as the computed staleness, or as two like-clock
> fields the capacity_slots view compares) — NOT node-clock fast_source_ts vs DB-clock
> heartbeat_ts. The build MUST NOT adopt the falsifier's literal fix of stamping
> fast_source_ts/slow_source_ts with DB now(), because that DEFEATS frozen-exporter
> detection (RFC gate bullet 1 needs the source's own measurement time, which stops
> advancing when the exporter freezes). RESTATE OQ-C to name which clock each timestamp
> uses. Required tests: (i) gate-test-A MUST additionally prove a node↔DB clock skew of
> several × half_life does NOT spuriously decay a FRESH slot; (ii) gate-test-B MUST still
> prove a genuinely frozen source_ts (no writer touching the row) decays to stale within
> k × half_life.
> **Verification gate:** skew + frozen-source tests: a several-×-half_life node↔DB skew
> leaves a fresh slot measured; a frozen source_ts decays to stale within k × half_life.
> **Expected stage:** design_revision_then_build_verify. **final_review_required: true.**

> **BC3** *(severity medium, kind policy, binding: false, source_finding `f_probe_none_crash`)* —
> Slice 1 MUST guard the live_slowdown_factor computation against probe_ms is None
> (failed/timed-out decode_probe; and the ollama-ondemand residency-only path which by the
> plan's own design returns probe_ms=None on every tick) and against cold_probe_ms
> None-or-0: write NULL / a sentinel with capacity_source reflecting absence and NEVER
> raise, and keep the computation inside the savepoint-guarded fault-isolation boundary so
> even an unexpected raise cannot sink the liveness UPSERT. Define explicitly that an
> ollama-ondemand slot (no decode baseline) yields a well-formed companion row
> (live_slowdown_factor NULL / capacity_source set accordingly), not a crash and not a
> silently-absent row. Required hermetic tests: (i) a failed/None probe leaves the heartbeat
> tick complete and the companion row well-formed; (ii) a cold loadable ollama-ondemand slot
> (probe_ms None) produces a well-formed companion row and is never force-loaded.
> **Verification gate:** probe-none tests: a None/0 probe_ms and a cold ollama-ondemand slot
> both yield a well-formed companion row with the heartbeat tick completing and liveness
> unaffected. **Expected stage:** design_revision_then_build_verify. **final_review_required: false.**

> **BC4** *(severity medium, kind policy, binding: false, source_finding `f_puller_companion_absent`)* —
> Slice 2 (or Slice 1, wherever CAPACITY_UPSERT first lands) MUST wire the companion-table
> CAPACITY_UPSERT into the PULLER write-path (heartbeat_all.py pull_write / the tick
> completion step), so pull-mode slots — including peecee, the host whose co-tenant
> motivates Principle 3 — actually get gpu_slots_capacity rows rather than only the legacy
> COALESCE fall-through. The puller's companion write MUST keep the same fault-isolation
> discipline (separate, savepoint-guarded) as the push-mode write, and MUST honor 'only the
> card-owning node writes its phantom' (per-PID attribution for peecee handled per the
> plan's existing proxy/deferral, not invented). Required integration test: running the
> puller driver populates gpu_slots_capacity for a pulled node (asserting the companion row
> is written, not just the liveness UPSERT).
> **Verification gate:** puller integration test: heartbeat_all.py's pull path writes a
> gpu_slots_capacity companion row for a pulled node. **Expected stage:**
> design_revision_then_build_verify. **final_review_required: false.**

> **BC5** *(severity low, kind policy, binding: false, source_finding `f_slice_deploy_order`)* —
> The revised plan MUST disambiguate that the slices are independently COMMITTABLE (each
> green under hermetic pytest, which applies all migrations) but NOT freely deploy-ordered:
> Slice 2 has a HARD deploy-order precondition on Migration 010 because mig_mode/ecc_mode
> ride the NON-savepoint-guarded gpu_slots liveness UPSERT, so deploying Slice 2 against the
> un-migrated schema fails the liveness UPSERT and ages slots out. State the DB→writer→reader
> apply order as the operative invariant (it already does in §2; tighten §1's 'independently
> committable … safe in either deploy order' so 'either order' is scoped to writer A-vs-B
> only). Do NOT move mig/ecc into the guarded companion write (they must bump epoch, which
> lives in the gpu_slots UPSERT by design) and do NOT add runtime column-existence probing
> (YAGNI for this DB-first, in-order operator deploy). Keep the holder's
> lowest-unused-migration-number guard.
> **Verification gate:** committable-vs-deployable disambiguation present; Slice 2's 010
> precondition stated explicitly; mig/ecc remain in the gpu_slots UPSERT epoch CASE.
> **Expected stage:** design_revision_then_build_verify. **final_review_required: false.**

### Operator follow-up directives (binding on the build run; from `OPERATOR_DECISION_BC1_override.md`)

- The **build MUST implement BC1** and add its **e2e `di_fleet.main()` 32k-vs-4k
  falsifying test** (a 32k request refused and a 4k request accepted on the SAME slot whose
  `effective_free` sits between the two headroom thresholds; `pick` + first-attempt claim +
  failover claim all receive non-default `max_context`).
- The **independent build verifier MUST confirm the BC1 e2e test AND the BC2 skew+frozen
  tests before `accept`.**
- **Preserve everything under "What survived"** and **do NOT re-open the settled RFC.** BC1
  is sourced at the di-fleet layer (registry `model_capacity` model→mib policy row +
  `_split_argv` argv) with **no engine import and no live-hardware read**; if neither source
  suffices, **escalate** rather than cross the `di --json` boundary or measure real GPUs.

---

# BUILD PLAN (committed)

The remainder of this document is the committed build plan — the holder's cycle-3
`BUILD_PLAN.md` (`holder-claude-opus-4.8-003`) with BC1–BC5 already folded into its slices
and §3 gate→test map, preserved here as the exact contract the build run executes.
